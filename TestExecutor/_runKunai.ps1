...
if ($localbackendlogfile) {
    ...
    invoke-expression ('cmd /c start powershell -Command {{ .\_runKunaiBackend.ps1 -logfile {0} 2>&1 > backendconsole.log}}' -f $localbackendlogfile)
} else {
...
if ($localfrontendlogfile) {
    invoke-expression ('cmd /c start powershell -Command {{ .\_runKunaiFrontend.ps1 -logfile {0} 2>&1 > frontconsole.log; read-host}}' -f $localfrontendlogfile)
} else {
